...
### TaskTableView.tsx
**Purpose**: Table view for task management with URL routing support
...
- **URL-Based Highlighting**: Selected task rows highlighted with cyan effects
- **Deep Linking**: Row selection updates URL with task ID
...
.selected-row {
  ...
  ring: 2px solid rgba(34, 211, 238, 0.2);
}
```

### DraggableTaskCard.tsx  
**Purpose**: Individual task card component with deep linking support
...
| `isSelected` | `boolean` | **NEW**: URL-based selection state |

**URL Integration**:
- **Visual Highlighting**: Selected cards show cyan glow effects
- **Deep Linking**: Card selection updates URL with task ID
- **Browser Navigation**: Maintains selection through back/forward navigation
...
#### DocsTab.tsx
**Purpose**: Project documentation management with URL routing support

**Features**:
- Document list with URL-driven selection
- Create/edit documents (disabled during migration)
- Version history
- Search functionality
- **Deep Linking**: Direct access to specific documents via `/projects/{id}/docs/{doc-id}`
- **Tab Navigation**: Auto-selects docs tab for `/projects/{id}/docs` URLs
- **Error Handling**: Graceful fallbacks for invalid document IDs

**URL Integration**:
- Document selection updates URL for shareable links
- Browser navigation maintains document selection
- Document not found redirects to docs tab with error logging
