...
### TaskBoardView.tsx
**Purpose**: Kanban board for task management with deep URL linking
...
- Task quick actions
- Real-time updates
- **Deep URL Navigation**: Direct linking to specific tasks with highlighting
- **Cross-Platform Copy**: Shift-click copies URL, regular click copies task ID
- **Auto-Scroll**: Automatically centers deep-linked tasks in viewport
- **Task Highlighting**: Vibrant cyan highlighting for URL-linked tasks
...
**URL Features**:
- `/projects/{projectId}/tasks/{taskId}?view=board` - Direct task linking
- Maintains view state in URL parameters
...
- Progressive loading with light/full content modes

### TaskTableView.tsx
**Purpose**: Table view for task management with enhanced URL navigation
...
- Bulk actions
- **Deep URL Navigation**: Direct linking to specific tasks with highlighting
- **Cross-Platform Copy**: Mobile-friendly "Copy URL" buttons with text labels
- **Auto-Scroll**: Automatically scrolls to and highlights deep-linked tasks
- **View Persistence**: Maintains table view in URL parameters

**URL Features**:
- `/projects/{projectId}/tasks/{taskId}?view=table` - Direct task linking in table view
- Cross-platform scrollbar fixes for Mac/Windows/iOS
...
#### DocsTab.tsx
**Purpose**: Project documentation management with deep URL linking and performance optimization

**Features**:
- Document list with horizontal scrolling
- Create/edit documents  
- Version history
- Search functionality
- **Deep URL Navigation**: Direct linking to specific documents (`/projects/{projectId}/docs/{docId}`)
- **Cross-Platform Copy**: Desktop shift-click for URLs, mobile "Copy URL" buttons
- **Light/Full Content Modes**: Fast loading with metadata-only mode, full content on demand
- **Auto-Scroll**: Automatically centers selected documents in viewport
- **Cross-Platform Scrollbars**: Fixed disappearing scrollbars on Mac with custom CSS

**Performance Optimizations**:
- Progressive loading: Light mode (metadata) → Full mode (content) on selection
- Document content caching prevents duplicate API calls
- Background loading for non-active documents
- Horizontal scroll optimization for document cards
...
### Navigation & Copy Utilities

#### Deep URL Navigation System
**Purpose**: Complete URL-based navigation for projects, tasks, and documents

**Key Features**:
- **React Router v6**: Enhanced parameter handling with `useParams()` and `useNavigate()`
- **URL State Management**: Route parameters, query parameters, and path-based tabs
- **Browser Integration**: Forward/back buttons, bookmarks, page refresh support
- **Progressive Loading**: Target content first, background loading for others

**URL Patterns**:
- `/projects/{projectId}` - Direct project access
- `/projects/{projectId}/tasks?view=table/board` - Task view navigation  
- `/projects/{projectId}/tasks/{taskId}?view=table/board` - Specific task linking
- `/projects/{projectId}/docs/{docId}` - Document deep linking

#### Cross-Platform Copy System
**File**: `src/utils/copyHelpers.ts`

**Purpose**: Mobile-compatible clipboard functionality with URL construction

**Key Functions**:
- `constructDeepLinkUrl()` - Builds URLs for projects, tasks, documents
- `copyToClipboardWithFallback()` - iOS/Android compatible clipboard with graceful fallbacks
- `handleCopyClick()` - Shift-click URL vs regular click ID copying
- `copyUrlToClipboard()` - Dedicated URL copying for mobile "Copy URL" buttons

**Cross-Platform Features**:
- **Desktop**: Shift-click copies full URL, regular click copies ID
- **Mobile**: Separate "Copy URL" buttons with proper text labels  
- **Error Handling**: Toast notifications for copy success/failure
- **Fallback System**: `document.execCommand('copy')` when modern Clipboard API fails

#### Platform Detection
**File**: `src/utils/platformDetection.ts`

**Purpose**: Detect mobile devices for adaptive UI behavior

**Usage**: Determines when to show mobile-specific copy buttons vs desktop shift-click behavior
